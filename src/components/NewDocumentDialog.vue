...
export type NewDocumentType = "markdown" | "word";
...
const emit = defineEmits<{
  ...
  create: [draft: NewDocumentDraft];
}>();

const { t } = useI18n();
const selectedDatabaseId = ref("");
const documentType = ref<NewDocumentType>("markdown");
...
function resetForm() {
  ...
  documentType.value = props.initialType;
  titleDraft.value = "";
  tagsDraft.value = "";
      ...
      try {
        ...
        tagNodes.value = usableExistingTagNodes(
          (tree.roots[0]?.children ?? []).filter(
            (node) => node.key !== ALL_DOCUMENTS_NODE_KEY,
          ),
        );
      } catch {
        tagNodes.value = [];
  ...
  >
    <div class="dialog-content">
      ...
      <fieldset class="field field--choices">
        <legend class="field-label">{{ t("newDocument.documentType") }}</legend>
        <div class="choice-row">
          <label class="choice">
            <input v-model="documentType" type="radio" value="markdown" :disabled="creating" />
            <span>{{ t("common.markdown") }}</span>
          </label>
          <label class="choice">
            <input v-model="documentType" type="radio" value="word" :disabled="creating" />
            <span>{{ t("common.word") }}</span>
          </label>
        </div>
      </fieldset>
